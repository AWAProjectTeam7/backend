...
const queries = {
    ...
    getVenueInfo: (venueID, callback) => {
        database.query('SELECT restaurant.ID, restaurant.name, restaurant.image, restaurant.address, restaurant.city, restaurant.openHours, restaurant.pricing, restaurantCategory.name AS category FROM restaurant, restaurantCategory WHERE restaurant.ID=? AND restaurantCategory.ID=restaurant.categoryID', [venueID], callback);
    },
