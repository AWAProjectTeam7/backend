...
const queries = {
    ...
    addToken: (token, callback) => {
        database.query('UPDATE user SET token=?, token_date=? WHERE userID=?', [token.value, token.expires, token.userID], callback);
    },
    ...
    findUser: (username, callback) => {
        database.query('SELECT token, token_date, userID FROM user WHERE email=?', [username], callback);
    },
